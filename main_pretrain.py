...
def get_args_parser():
    ...
    parser.add_argument('--blr', type=float, default=5e-5, metavar='LR',
                        help='base learning rate: absolute_lr = base_lr * total_batch_size / 256; default=1.5e-4')
    ...
    # Dataset parameters
    parser.add_argument('--dataset', default='imagenet-lt', type=str, help='dataset path')

    parser.add_argument('--output_dir', default='./pretrain_output_dir',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./pretrain_output_dir',
    ...
    parser.add_argument('--crop-min', default=0.6, type=float,
...
def main(args):
    ...
    if args.dataset == 'iNat18':
        print('train_dataset: iNat18')
        dataset_train = iNat18Dataset(
            '/data/iNat18',
            '/data/iNat18/train2018.json',
            TwoCropsTransform(transforms.Compose(augmentation1), 
                            transforms.Compose(augmentation2)))
    elif args.dataset == 'imagenet-lt':
        dataset_train = ImagenetLTDataset(
            '/data/imagenet',
            '/data/imagenet/ImageNet_LT_train.txt',
            TwoCropsTransform(transforms.Compose(augmentation1), 
                            transforms.Compose(augmentation2)))
